...
<div class="max-w-3xl mx-auto bg-white p-6 rounded shadow space-y-6">
    ...
    <form action="{{ route('admin.cars.store') }}" method="POST" class="space-y-4" enctype="multipart/form-data">
        @csrf
        ...
        <div>
            ...
            <select name="car_type" id="car_type"
                ...
                <option value="rental" {{ old('car_type') == 'rental' ? 'selected' : '' }}>Rental</option>
                <option value="sale" {{ old('car_type') == 'sale' ? 'selected' : '' }}>Sale</option>
            </select>
        </div>
        ...
        <div>
            ...
            <select name="make" id="make" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm px-4 py-2">
        ...
        <div>
            ...
            <input type="text" name="model" id="model" value="{{ old('model') }}" placeholder="e.g. Corolla, Mustang"
        ...
        <div>
            ...
            <input type="number" name="year" id="year" value="{{ old('year') }}" placeholder="2023"
                class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        ...
        <div>
            ...
            <input type="text" name="color" id="color" value="{{ old('color') }}" placeholder="Red, Blue, Black"
                class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        ...
        <div>
            ...
            <input type="number" name="mileage" id="mileage" value="{{ old('mileage') }}" placeholder="e.g. 12000"
                class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        ...
        <div>
            ...
            <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">{{ old('description') }}</textarea>
            @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
        </div>

        <!-- Rental Price -->
        <div id="rental_fields" class="{{ old('car_type') == 'sale' ? 'hidden' : '' }}">
        ...
        <!-- Sale Price -->
        <div id="sale_fields" class="{{ old('car_type') != 'sale' ? 'hidden' : '' }}">
        ...
        <div>
            ...
            <input type="file" name="images[]" accept="image/*" multiple class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
            @for ($i=0; $i<5; $i++)
                <input type="url" name="image_urls[]" placeholder="Or image URL" class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
                @endfor
        </div>

        <div class="flex space-x-2">
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow-sm">Add Car</button>
            <a href="{{ route('admin.cars.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded shadow-sm">Cancel</a>
        </div>
    </form>
</div>
