...
<div class="bg-white p-6 rounded shadow space-y-6">
    ...
    <form action="{{ route('admin.sales.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="buyer_name" value="{{ old('buyer_name') }}" class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        </div>
        ...
        <div>
            ...
            <input type="date" name="sale_date" value="{{ old('sale_date') }}" class="mt-1 block w-full border-gray-300 rounded shadow-sm px-4 py-2">
        </div>

        <div class="flex space-x-2">
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Add Sale</button>
